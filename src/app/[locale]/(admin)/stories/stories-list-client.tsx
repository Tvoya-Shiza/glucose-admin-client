 ...
 *
 * URL state: page, page_size, q, status, category_id, sort, order. Filter changes
...
export function StoriesListClient() {
    ...
    const queryKey = useMemo(
        () => ['admin.stories.list', { page, page_size, status, category_id, q, sort, order }] as const,
            ...
            <header className='flex items-center justify-between p-6'>
                ...
                <div className='flex items-center gap-2'>
                    <Button asChild variant='outline'>
                        <Link href={`/${locale}/stories/categories`}>{t('categories_title')}</Link>
                    </Button>
                    {isAdmin ? (
