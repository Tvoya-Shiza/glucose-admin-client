...
import type { TopQuizRow } from '@/lib/quizzes/types';

interface Props {
    rows: TopQuizRow[];
}

export function TopQuizzesTable({ rows }: Props) {
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>{t('results_stats_top_quizzes_title')}</CardTitle>
            </CardHeader>
            <CardContent>
                {rows.length === 0 ? (
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead>{t('results_stats_top_col_quiz')}</TableHead>
                        ...
                        <TableBody>
                            {rows.map((r, i) => {
                                const label = r.title_kz?.trim() || `#${r.quiz_id}`;
                                return (
                                    <TableRow key={r.quiz_id}>
                                        <TableCell className='text-muted-foreground tabular-nums text-sm'>{i + 1}</TableCell>
                                        <TableCell>
                                            <Link href={`/${locale}/quizzes/${r.quiz_id}`} className='hover:underline'>
                                                {label}
                                            </Link>
                                        </TableCell>
