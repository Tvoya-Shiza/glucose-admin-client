...
import type { BannerCategoryRow } from '@/lib/banners/types';
...
function CategoryDialog({
    ...
    category: BannerCategoryRow | null;
}) {
    ...
    const mutation = useMutation({
        mutationFn: (values: CategoryValues) =>
            isEdit
                ? updateBannerCategory(category!.id, values)
                : createBannerCategory(values),
        onSuccess: () => {
            toast.success(t('saved'));
            qc.invalidateQueries({ queryKey: ['admin.banners.categories'], exact: false });
            onOpenChange(false);
        },
        onError: (err: unknown) => {
...
/**
 * BAN-02 — banner categories list + CRUD.
 *
 * Mirrors Stories CategoriesClient (Plan 02). No bulk actions; small flat surface
 * (D-15). Delete attempts hard delete; on 400 'banners.category_in_use' the
 * Error.message is surfaced via toast (the backend BadRequestException uses the
 * i18n key as the message).
 */
export function CategoriesClient() {
    ...
    const [editing, setEditing] = useState<BannerCategoryRow | null>(null);
    ...
    const del = useMutation({
        mutationFn: (id: number) => deleteBannerCategory(id),
        onSuccess: () => {
            ...
            <header className='flex items-center justify-between p-6'>
                <div>
                    ...
                    <p className='text-muted-foreground text-sm'>
                        <Link href={`/${locale}/banners`} className='hover:underline'>
                            ← {t('list_title')}
                        </Link>
                    </p>
                </div>
